...
from project_guidance import explain_one_liner, project_guidance
...
def concise_digest_from_raw(
    raw: dict | None,
) -> tuple[list[str], list[str], list[str], list[str], list[str], int]:
    """Return primary_pick lines, secondary_picks, rest_candidates, new_preview, hn_items, extra_new_count."""
    if not raw:
        return [], [], [], [], [], 0
    ...
    if scores:
        ...
        what, why, action = project_guidance(full_name, top.get("tags") or [], description)
        primary_pick = [
            f"1. {full_name}",
            f"   是什么: {what}",
            f"   为什么看: {why}",
    ...
    for idx, item in enumerate(scores[1:3], 2):
        repo = repos.get(item["full_name"], {})
        full_name = item["full_name"]
        url = repo.get("html_url") or f"https://github.com/{full_name}"
        description = repo.get("description") or ""
        line = explain_one_liner(full_name, item.get("tags") or [], description, limit=64)
        secondary_picks.append(f"{idx}. {full_name} — {line} → {url}")
    ...
    for idx, item in enumerate(scores[3:7], 4):
        ...
        description = repo.get("description") or ""
        line = explain_one_liner(item["full_name"], item.get("tags") or [], description, limit=56)
        rest_candidates.append(f"{idx}. {item['full_name']} — {stars:,}★, {tags} · {line}")
    ...
    return primary_pick, secondary_picks, rest_candidates, new_preview, hn_items, extra_new_count
...
def build_email_body(test_results: list[StepResult], radar_result: StepResult, report_text: str) -> str:
    ...
    raw = load_latest_raw()
    primary_pick, secondary_picks, rest_candidates, new_preview, hn_items, extra_new_count = concise_digest_from_raw(raw)
